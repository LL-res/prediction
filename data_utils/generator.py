...
def generate(base,  random_range, amplify_factor=1,total_size = -1,upbound = sys.maxsize,downbound = 0):
    """
    Parameters:
    ...
    if total_size == -1:
        ...
        for i in range(total_size):
            ...
            total_set_raw.append(element)
    for i in range(len(total_set_raw)):
        if total_set_raw[i] > upbound:
            total_set_raw[i] = upbound
        if total_set_raw[i] < downbound:
            total_set_raw = downbound
