...
class result:
    ...
    def save(self):
        ...
        if os.path.exists(results_path) is False:
            results_header = ['model_type', 'container_id','data_type','mse', 'train_time_consumed(s)','predict_time_consumed(ms)','real_value','predict_value']
            with open(results_path, mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(results_header)
        with open(results_path, 'a', newline='') as file:
            csv_writer = csv.writer(file)
            data_to_write = [self.model_type,self.container_id,self.data_type,self.mse,self.train_time_consumed,self.predict_time_consumed,self.real_value,self.predict_value]
...
class resultBuilder:
    ...
    def set_real_value(self,real_value):
        self.result.real_value = real_value
        return self

    def set_predict_value(self,predict_value):
        self.result.predict_value = predict_value
        return self
